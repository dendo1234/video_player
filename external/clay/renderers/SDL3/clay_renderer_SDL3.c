...
#else
	SDL_Vertex* vertices = (SDL_Vertex*)_malloca(sizeof(SDL_Vertex) * totalVertices);
	int* indices = (int*)_malloca(sizeof(int) * totalIndices);
#endif
...
#else
        SDL_FPoint* points = (SDL_FPoint*)_malloca(sizeof(SDL_FPoint) * (numCircleSegments + 1));
#endif
...
void SDL_Clay_RenderClayCommands(Clay_SDL3RendererData *rendererData, Clay_RenderCommandArray *rcommands)
{
    for (size_t i = 0; i < rcommands->length; i++) {
		...
		SDL_Log("1: %d", rcmd->commandType);
        SDL_Log("2: %d", CLAY_RENDER_COMMAND_TYPE_RECTANGLE);
